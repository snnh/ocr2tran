...
- `minTextLength` / `minConfidence`：按文本长度和 OCR 置信度过滤。`minConfidence` 只作用于 OCR 后端提供了大于 `0` 置信度的结果。
...
- `detLimitSideLen`：检测输入长边限制。
- `detThreshold` / `boxThreshold`：检测阈值。
- `unclipRatio`：检测框外扩比例。
- `minBoxSize`：过滤过小文本框。
