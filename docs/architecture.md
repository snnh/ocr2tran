...
4. 调用 OCR 后端：
   - `onnx`：内置 ONNX Runtime，加载检测模型和识别模型。
   - `paddleCli`：启动 `paddleocr.exe` 或兼容包装程序。
   - 其他值：进入 `noop` 调试 OCR。
5. 对 OCR 结果执行文本后处理：清洗控制字符、过滤低置信度/小框/噪声块、合并邻近文本、去除重复框。
6. 得到 `TextRegion` 列表，每项包含屏幕坐标、原文、译文和置信度。
7. 覆盖层按 `TextRegion.Bounds` 在原屏幕位置绘制文本。

## 翻译流程

翻译开启时，协调器逐条文本调用翻译后端。缓存 key 包含 provider、源语言、目标语言和原文。相同文本再次出现时直接复用译文。
...
- ONNX 后端实现 DB 风格检测 + CTC 识别，当前文本框为轴对齐矩形。
- ONNX 方向分类模型可导入但尚未参与旋转判定。
